...
- Windows assets attached from `main` are Microsoft Store-oriented MSIX packages generated by [`scripts/build-msix.mjs`](../scripts/build-msix.mjs) from the repository's Tauri release build output.
- MSIX packages are signed during CI with an auto-generated self-signed certificate. Microsoft re-signs the package upon Store publication. A CA-trusted certificate is not required for Store submission.
- Microsoft Store submission (Partner Center upload) remains an operator responsibility. Do not publish artifacts built outside the repository's reviewed source tree.
